...
public partial class SelectPlayerNames : CanvasLayer
{
	...
	public override void _Ready()
	{
		...
		_signals.SelectFirstEmpty += SelectFirstEmpty;

		_sceneSwitcher = GetNode<SceneSwitcher>("/root/SceneSwitcher");

		_playerInputs = GetNode<VBoxContainer>("PlayerInputs");

		if (Global.playerNames.Count > 0) {
			List<string> playerNames = Global.playerNames;
			...
			for (int i = 2; i <= playerNames.Count; i++) {
				Node nameInput = NodeCreator.getNameInput(i, playerNames[i-1]);
	...
	private void AddPlayerName(string name, int index) {
		if (index >= 0 && index < Global.playerNames.Count) Global.playerNames[index] = name;
		else Global.playerNames.Add(name);
	}
	...
	private void OnContinuePressed(){
		int amount_players= _playerInputs.GetChildCount();
		if (amount_players == 2){
			Node popUp = NodeCreator.getPopUp("In je eentje spelen \n word lastig denk ik");
			AddChild(popUp);
			return;
		}
		...
		if (emptyInputs.Count > 0){
			Node popUp = NodeCreator.getPopUp("Zal je niet iedereen ff \n een naam geven?");
			AddChild(popUp);
		}
		else {
			AddPlayerNames();
			_sceneSwitcher.SwitchScene("res://Windows/ChooseGame.tscn");
		}
		
	}

	private void AddDeleteButton(int index){
		Node deleteButton = NodeCreator.getDeleteButton();
	...
	private void AddPLayerInput(){
		if (IsInstanceValid(_playerInputs) && _playerInputs.GetChildCount() <= 8) {
			...
			Node nameInput = NodeCreator.getNameInput(index);
			_playerInputs.AddChild(nameInput);
	...
	private void DeletePlayerInput(Node node){
		...
		if (child_count < 8){
			...
			if (!addButtonVisible){
				Node addButton = NodeCreator.getAddButton();
